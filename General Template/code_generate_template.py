...
'''
#specify the tensorflow model (.h5) here 
...
desired results
'''
#give the names of files from dataset to be read as follows
...
for data in datalist:
    ...
    giving it to model.
    Perform the same preprocessing as you did during training
    Finally, load the input into the variable inp with dimensions as
    [channels, height, width] format and as a numpy array
    '''
    
    inp=[] #this variable is the input data for model, after preprocessing
    #it should be a numpy array with dimensions as specified before
...
for idx in range(len(model.layers)):
    ...
    if a.find('conv2d')!=-1:
        ...
        ll=np.max(maxval[idx])
        ll=nextpow2(math.ceil(ll))
        if idx==0:
            f.write('float sa0='+str(127/ll)+';//multiply input by sa0 and give as input to inference model\n')
...
f.write("/* the input value should be in data variable. To obtain this, multiply\n")
f.write("the floating point input by sa0 variable, which is stored in weight files\n")
f.write("already and round off the value and save it in data variable*/\n")
itr=0
for idx in range(len(model.layers)):
    a=(model.get_layer(index=idx).name)
    if a.find('conv2d')!=-1:
