...
q7_t FC_OUT[IP1_OUT_DIM],SOFT_OUT[IP1_OUT_DIM];
//Buffers
q7_t col_buffer1[2000];
char predict;
int loc;
//quantized values, i.e normalised and multiplied by sa0 variable already
q7_t data[]={
...
int max_fout=SOFT_OUT[0];
loc=0;
	for(int i=0; i<IP1_OUT_DIM;i++){
		if(SOFT_OUT[i]>max_fout)
		{
			max_fout=SOFT_OUT[i];
			loc=i;
		}
